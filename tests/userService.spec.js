const { executeCreate, executeUpdate, executeDetail, executeLogin } = require('../src/services/userService');
...
describe('User Service - ExecuteCreate', () => {
  ...
  test('Success create user', async () => {
    ...
    expect(userRepository.insert).toHaveBeenCalledWith(userData.nome, userData.email, userData.senha);
...
describe('User Service - ExecuteUpdate', () => {
  ...
  test('Success update user', async () => {
    ...
    hash.mockReturnValue(userData.senha);
    userRepository.update.mockReturnValue([
      {
        id: 1,
        nome: userData.nome,
        email: userData.email,
    ...
    expect(userRepository.update).toHaveBeenCalledWith(userData.id, userData.nome, userData.email, userData.senha);
  });
...
describe('User Service - ExecuteUserDetail', () => {
  ...
  test('Success detail user', async () => {
    ...
    const response = await executeDetail(userData.id);
  ...
  test('Throw an exception because user not exists', async () => {
    ...
    try {
      await executeDetail(userData.id);
    } catch (error) {
      expect(userRepository.findById).toHaveBeenCalledTimes(1);
      expect(error).toBeInstanceOf(AppError);
